...
A prefix, a floor range, and a units-per-floor range (e.g. prefix `W`, floors 2–29, units 1–10 per floor generates `W201`...`W2910`, matching the existing DuEast unit-numbering convention) generate a batch of codes, 4–8 characters long (configurable). A "Character Types" dropdown controls whether codes use letters + numbers (default), letters only, or numbers only — each option skips its own easily-confused characters (I/O from letters, 0/1 from numbers). There's no symbols option, since unit codes are typed on a phone keyboard and adding symbols would mean an extra keyboard-layout switch for no real security benefit (codes are compared case-insensitively, so letter case doesn't add entropy either).

**Confirming a bulk generate deletes existing codes in the range you specify and replaces them with the new batch** — there's no partial/preview-checkbox mode, and it's a single atomic delete+insert (safer than a client-side merge that could partially fail). The preview screen shows how many existing codes fall inside the range and will be deleted before you confirm.

As of 2026-08-12, that delete is scoped to the range you're generating, not the whole building. This matters for any building with more than one tower sharing an address — DuEast's "225 Sumach Street" has West Tower (`W201`-`W2910`) and East Tower (`E201`-`E1110`) on one address, so generating just the West range only ever touches West's codes; East's are left alone. (Earlier versions of this feature wiped the entire building on every generate, which silently deleted the other tower's codes — if you're on an older install, run the patch below.)

If you actually want to clear a building's entire code list — e.g. re-numbering it from scratch with a new tower layout — use the separate **🗑 Wipe All Codes** button in the same modal. It has its own confirmation step, and always scopes to exactly one building, never a whole tenant. Run [patch-scoped-bulk-regenerate-and-wipe-codes.sql](/Users/sameerbhaidani/Documents/VIsitors%20Parking%20App/patch-scoped-bulk-regenerate-and-wipe-codes.sql) once in the Supabase SQL Editor to pick up both the scoped-delete fix and the new `wipe_unit_codes` RPC on an existing install.

Tenant Admin staff can still add, edit, or remove individual unit codes one at a time from the "🔑 Unit Codes" tab — only the bulk range-generator and Wipe All moved to Platform Admin.
